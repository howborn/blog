...
假若，你是某个国内电商平台的商品中心项目负责人。突然有一天，接到了一个这样的需求：商品在原人民币价格的基础架构上，须支持卢比（印度）价格。
...
最终，为了系统的可扩展性，我们选择了方案 ②，主要改造了商品中心，并且商品价格支持了多币种。

我们的业务系统主要分为 API 和 Service 项目，API 暴露出 HTTP 接口，API 与 Service 和 Service 与 Service 之前使用 RPC 接口通信。方案 ② 主要需要解决 [透传地区标识](#透传地区标识) 和 [支持多币种价格](#支持多币种价格) 这 2 个问题。

### 透传地区标识

由于商品中心涉及到价格的接口繁多，不可能对每个接口都增加地区标识的参数。所以我们弄了一套调用链路透传地区标识的机制。

#### 机制原理

![透传地区标识](https://img2.fanhaobai.com/2019/02/multi-currency-price/)
