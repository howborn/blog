...
[拆分算法]()：我并没有找到业界的通用算法，但红包拆分算法应该是拆分金额要看起来随机，最好能够服从正态分布，可以参考 [微信](https://www.zybuluo.com/yulin718/note/93148) 和  [@lcode](https://www.fanhaobai.com/2017/02/reward.html) 提供的红包拆分算法。
...
在测试红包过程中，我们遇到了一个不算问题的问题。由于运营将测试资金充值到线上的微信公众号中，所以还必须使用线上公众号授权的 openid 进行测试（否则会出现 openid 跟公众号不一致问题）。为了让线上公众号能够授权到测试环境，我们测试时在线上的微信授权回调地址新增一个参数做区分，将带有`to=feature`参数的请求引流到测试环境，其他线上正常流量还是保持不变，匹配规则如下：
